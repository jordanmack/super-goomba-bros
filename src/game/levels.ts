...
export function isSolidTile(id: number) {
  return (id >= 16 && id <= 34) || (id >= 81 && id <= 94) ||
    (id >= 97 && id <= 108) || id === 136 || id === 137 ||
    id === 192 || id === 193 || id === 196;
}
export function themeFor(area: Area) {
  return area.type !== "overworld" ? area.type : area.id === "2d" ? "snow" : area.palette === "night" ? "night" : "day";
}
...
export function terrainRects(area: Area, offset = 0): LevelRect[] {
  ...
  for (let x = 0; x < area.width; x++) {
    ...
    for (let y = 2; y <= 18; y++) {
      ...
      const solid = y < 18 && isSolidTile(tile) && !(y < 15 && interactive.has(`${x},${y}`));
      if (solid && top < 0) top = y;
      if (!solid && top >= 0) {
        columns.push({ x: offset + x * 32, y: MAP_TOP + top * 32, width: 32, height: (y - top) * 32 });
        top = -1;
      }
    }
  }
  ...
  for (const column of columns) {
    const previous = rectangles.find(r => r.x + r.width === column.x && r.y === column.y && r.height === column.height);
    if (previous) previous.width += 32; else rectangles.push({ ...column });
  }
  if (isSolidTile(area.tiles[14][0])) rectangles.push({ x: offset - 300, y: TUNING.groundY, width: 300, height: 160 });
  if (isSolidTile(area.tiles[14].at(-1)!)) rectangles.push({ x: offset + area.width * 32, y: TUNING.groundY, width: 300, height: 160 });
  return rectangles;
}
...
export function areaGaps(area: Area, offset = 0): [number, number][] {
  ...
  for (let x = 0; x <= area.width; x++) {
    ...
    if (!gap && start >= 0) { gaps.push([offset + start * 32, offset + x * 32]); start = -1; }
  }
  return gaps;
}
